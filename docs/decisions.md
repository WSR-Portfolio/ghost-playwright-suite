...
The hardware has since changed. Ghost and its companion services (including Mailpit, now at `http://<mailpit-host>:8025`) were migrated onto a dedicated host — an Intel i7 with 16 GB of RAM. The difference is dramatic and measured, not assumed: the foundational admin login (AU-001) dropped from ~50s under CI load on the NAS to ~4s, and a full sequential suite run fell from ~18.8 min in CI (when it passed at all) to ~3.0 min. On the NAS the CI runner was a container sharing the box with Ghost, so the runner and its headless Chromium starved the application under test; the new host removes that contention entirely.
...
2. **Reset the slate each run.** A Playwright `globalSetup` (`tests/global-setup.ts`) clears the `brute` table (`DELETE FROM brute`) before any project starts, so no run inherits another's lockout state. It connects with a dedicated MySQL user (`brute_reset_user`) granted **`DELETE` on the `brute` table only** — if those credentials leak they cannot read or modify content or members. The credentials come from `DB_*` env vars; when they are absent (e.g. a dev machine that cannot reach the host's MySQL) the setup logs a warning and no-ops rather than failing, relying on the tuned config alone.
...
Because the suite now relaxes a security control, it also *positively tests* that the control still works: RL-001 (`tests/api/rate-limit.spec.ts`) deliberately drives the member sign-in endpoint past `freeRetries` and asserts the limiter returns a 429. This is the inverse of every other auth test, so it is isolated in a dedicated `rate-limit` project that depends on `main` and `member` — guaranteeing it runs last and alone, never concurrently with a sign-in test that shares the per-IP `global_reset` bucket. It resets `brute` immediately before (deterministic threshold) and after (clean slate) itself, and skips when DB credentials are absent, since it must be able to undo the lockout it creates. Tuning the limiter and verifying it still fires are two halves of the same decision: relax it for legitimate test volume, but prove it still defends against abuse.
